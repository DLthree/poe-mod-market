...
import { kindOfType } from './item-kinds.mjs'
...
export function price (db, { league, type, rarity, mods: wanted = [], lookbackHours, tradeWindow,
  ...
  return {
    ...
    next,
    // The kind follows from the type this endpoint was already asked about, so
    // it is looked up rather than added as a parameter. A type no kind claims
    // yields a null link, which is the honest answer: we cannot say what such a
    // search would have to pin.
    trade: tradeUrl({ league, kind: kindOfType(type), type, rarity, mods: wanted, tradeWindow }),
