...
// NOT A STAT GROUP. The corrupted filter lives in query.filters.misc_filters, so
// `pinned` returns it apart from `groups` and both callers merge it into the
// query's own filters.
//
// WHICH cells exclude it is lib/poe2.mjs pricesIncludeCorrupted, shared with
// lib/summary.mjs. That sharing is the point: until 2026-09-10 only the summary
// knew the rule, so a magic search collected corrupted listings, the summary
// discarded them, and the trade link opened a market whose cheap end the
// published price had excluded — while calling itself exact. On Sapphire magic
// the blank cell published 4 exalted with a 2-exalted corrupted listing at the
// top of the link.
const corruptedFilters = (rarity) => pricesIncludeCorrupted(rarity)
  ? {}
...
export const ITEM_KINDS = {
  tablet: {
    ...
    pinned (type, rarity) {
      const filters = corruptedFilters(needRarity('tablet', rarity))
  ...
  jewel: {
    ...
    pinned: (type, rarity) => ({
      groups: [noDesecrated()],
      filters: corruptedFilters(needRarity('jewel', rarity)),
      missing: []
    }),
