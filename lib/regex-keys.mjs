// PHASE 2. Turns chosen modifiers into a PoE2 stash-search string. Nothing in
// here is particular to an item kind: it joins fragments, and a fragment is a
// piece of a modifier's own wording whatever carries it.
//
...
// item type or its rarity: you are looking at one type's modifier list, and
...
export function stashRegex ({ statIds = [], fragments = {}, mode = 'any' }) {
  const keyed = []
  const unkeyed = []
