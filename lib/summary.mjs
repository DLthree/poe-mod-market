...
import { inExalted, validateExchange } from './exchange.mjs'
import { pricesIncludeCorrupted } from './poe2.mjs'
...
// WHY a magic or normal cell drops its corrupted rows is in lib/poe2.mjs, beside
// the rule itself. The rule is shared because lib/item-kinds.mjs now keeps those
// listings out of the search as well, and two copies of it drifted once: the
// search collected them, this discarded them, and the trade link showed them.
//
// This filter still has to run. It reads rows collected before the search learned
// to exclude them, and the archive keeps every one of those.
const plain = (rows, rarity) =>
  pricesIncludeCorrupted(rarity) ? rows : rows.filter(r => !r.corrupted)
...
// What a modifier costs against a blank tablet of the same cell, as a multiple.
// Null when the two cannot be compared at all: no price of its own, a baseline
// in another currency, or a blank tablet that floors at nothing. Null means
// "cannot say" and never "worthless" — lib/bands.mjs turns a ratio into a band,
// and a low ratio is a small number there, not a null here.
//
// The sample size is deliberately not consulted. See lib/bands.mjs.
//
// Both sides are taken to exalted first, so a modifier priced in divine can be
// compared to a blank tablet priced in exalted. Where the two currencies match
// the rate cancels and the ratio is exactly what it was before.
function ratioOf (m, baseline, rates) {
  if (!m.priced || m.floor === null) return null
  const f = inExalted(m.floor, m.currency, rates)
  const b = inExalted(baseline.value, baseline.currency, rates)
  if (f === null || b === null || !(b > 0)) return null
  return f / b
}

// True when the configured rate actually moved a number on this row, so the
// page can say that the comparison rests on a standing guess. A ratio between
// two divine prices does not need the rate — but `delta` is denominated in
// exalted, so it does.
const usesRate = (m, baseline) =>
  (m.priced && m.floor !== null) &&
  (m.currency !== 'exalted' || baseline.currency !== 'exalted')

// How much evidence stands behind a modifier's price. Reported beside the
// price, never folded into it: a legitimately rare modifier early in a league
// is thinly evidenced AND genuinely dear, and collapsing those two facts into
// one verdict hid exactly the modifiers worth finding.
const fewSamplesFor = (m, walk) =>
  m.matches < walk.minListings || m.sellers < walk.minSellers
...
export function cellSummary (db, { league, type, rarity, lookbackHours, config, textFor,
  ...
  for (const hash of asked) {
    ...
    // What the modifier is WORTH over a blank tablet, always in exalted. A
    // ratio hides the size of the prize: "x10" on a 1 ex baseline is 9 exalted,
    // while "x1.4" on a 100 ex baseline is 40.
    //
    // Exalted rather than "the cell's own currency", which is what it used to
    // be: on a divine-priced cell that made walk.minAdds, a threshold written
    // and documented in exalted, compare against a divine number, so a modifier
    // adding one divine failed a ten-exalted test it clears a hundred times
    // over. A single denomination is the only way that threshold means one
    // thing everywhere.
    const fEx = inExalted(f.value, f.currency, config.exchange)
    const bEx = inExalted(baseline.value, baseline.currency, config.exchange)
    const delta = (f.value > 0 && baseline.value > 0 && fEx !== null && bEx !== null)
      ? Number((fEx - bEx).toFixed(2))
      : null
  ...
  // These are validated here even though lib/bands.mjs is what applies them:
  // this is the last place that holds the config before it is published, and an
  // absent threshold would reach the page as NaN, where every comparison is
  // false and the whole list goes one flat colour. That reads as a disappointing
  // market rather than as a broken build. A missing setting is a fault.
  for (const [name, value] of Object.entries(bands)) {
    if (!(value > 0)) {
      throw new Error(
        ...
        `item of the cell a modifier must cost to be called ${name}.`)
    }
  }
  if (bands.high < bands.mid) {
    throw new Error(
      `config.walk.highVsBlank (${bands.high}) is below midVsBlank (${bands.mid}), ` +
      'so no modifier could ever be high without also being mid.')
  }
  // Zero is allowed and means "ratio only".
  if (!(config.walk.minAdds >= 0)) {
    throw new Error(
      `config.walk.minAdds must be a number of zero or more, got ` +
      `${JSON.stringify(config.walk.minAdds)}. It is how much a modifier must add ` +
      'over a blank item of the cell, in exalted, on top of clearing a band.')
  }
  // These no longer suppress anything, so a missing one would not blank the
  // page — it would silently call every sample sufficient, which is worse,
  // because the reader would never learn the evidence was never checked.
  for (const name of ['minListings', 'minSellers']) {
    if (!(config.walk[name] >= 0)) {
      throw new Error(
        `config.walk.${name} must be a number of zero or more, got ` +
        `${JSON.stringify(config.walk[name])}. It sets fewSamples, which reports how ` +
        'much evidence stands behind a price and never changes the price itself.')
    }
  }
  validateExchange(config.exchange)
  const typical = typicalFloor(out, baseline.currency)
  for (const m of out) {
    m.affixRatio = ratioOf(m, baseline, config.exchange)
    m.fewSamples = fewSamplesFor(m, config.walk)
    m.assumedRate = usesRate(m, baseline)
  }

  // Dearest first, plainly. The band is NOT a sort key: ordering by it put a
  // modifier at 18x the blank tablet below one at 3x, because the dear one had
  // four sellers and was therefore called null, and the list then read as
  // though it had been sorted as text.
  //
  // Dearest is measured in exalted, so a one-divine modifier reads above a
  // thirty-eight-exalted one instead of below it. It used to sort on the raw
  // amount within one currency and push every other currency down the list,
  // which put the dearest rows on Overseer and Temple beneath filler.
  //
  // A currency the rate table has no entry for still cannot be placed among
  // these, so it sorts after them, ahead of nothing but the unpriced. Within
  // each group, the more widely listed modifier reads first.
  const worth = (m) => (m.priced ? inExalted(m.floor, m.currency, config.exchange) : null)
  const group = (m) => (worth(m) !== null ? 0 : m.priced ? 1 : 2)
  out.sort((a, b) =>
    group(a) - group(b) ||
    (group(a) === 0 ? worth(b) - worth(a) : 0) ||
    b.sellers - a.sellers ||
    b.matches - a.matches)
