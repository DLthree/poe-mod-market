...
function qualityOf (m, baseline, bands, walk) {
  ...
  // where a blank tablet costs 1 exalted, 2.1x it is 2.1 exalted, and a
  // modifier "worth twice the tablet" is worth about nothing. Irradiated rares
  // banded six modifiers high on a tablet the whole market prices at 1 exalted.
  //
  // So a modifier must also add real money. This only bites where the blank is
  // cheap: at 10 exalted it takes Irradiated rare from 6 high to 0 and Overseer
  // magic from 24 to 10, and changes nothing at all on Abyss, Breach, Ritual or
  // Temple, whose blanks are dear enough that the ratio was already the harder
  // test.
  //
  // The threshold is in the CELL'S OWN currency, which is exalted on every cell
  // we have ever collected. A cell whose baseline came back in divine would
  // read this as 10 divine — worth noticing rather than guarding against, since
  // no such cell exists and inventing a conversion is what this codebase
  // refuses to do everywhere else.
  if (m.floor - baseline.value < walk.minAdds) return null
...
export function cellSummary (db, { league, type, rarity, lookbackHours, config, textFor,
  ...
  // Absent, this would compare against NaN and every modifier would come back
  // unbanded — the page would go uniformly grey and look merely disappointing
  // rather than broken. Zero is allowed and means "ratio only".
