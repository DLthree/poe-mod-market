...
export function cellSummary (db, { league, type, rarity, lookbackHours, config, textFor,
  ...
  if (!(config.walk.minAdds >= 0)) {
    throw new Error(
      ...
      "over a blank tablet, in the cell's own currency, on top of clearing a band.")
  }
  ...
  for (const m of out) {
    m.affixRatio = ratioOf(m, baseline)
  ...
  //
  // A price in a currency the baseline does not use cannot be placed among
  // these at all — that is the same refusal to invent an exchange rate that
  // holds everywhere else here — so those sort after the comparable ones,
  // ahead of nothing but the unpriced. Within each group, the more widely
  // listed modifier reads first.
  const comparable = (m) => m.priced && m.floor !== null && m.currency === baseline.currency
  const group = (m) => (comparable(m) ? 0 : m.priced ? 1 : 2)
  out.sort((a, b) =>
    group(a) - group(b) ||
    (group(a) === 0 ? b.floor - a.floor : 0) ||
    b.sellers - a.sellers ||
    b.matches - a.matches)
