...
import { USES_IMPLICIT, MIN_USES } from './item-kinds.mjs'
...
// MIN_USES is in lib/poe2.mjs beside USES_IMPLICIT, because the link the page
// builds has to ask for the same item this query asks for.
//
...
export const affixQuery = (type, hash, tradeWindow, rarity) => {
  if (!rarity) {
    throw new Error(
  ...
  const q = base(type, rarity, tradeWindow)
...
export async function sweepAffixes ({ client, db, index, league, types, rarities, perCell,
  ...
  for (const type of types) {
    for (const rarity of rarities) {
      ...
      for (const hash of hashes) {
        ...
        onCell(cell)
        const r = await runCell({ client, db, index, perCell,
          query: affixQuery(type, hash, tradeWindow, rarity),
