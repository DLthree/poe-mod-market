...
async function boot () {
  // The league is not a constant here any more than it is on poe.re: ask the
  // server which leagues it holds, and open the one it was started with. A new
  // league arrives every few months and the page must not need a restart to
  // show it.
  const { leagues, default: started } = await fetch('/api/leagues').then(r => r.json())
  const picker = $('#league')
...
async function loadLeague (league) {
  const [eco, fragments] = await Promise.all([
    fetch('/' + encodeURI(`tablet/eco_${league}_Tablet.json`)).then(r => r.json()),
    fetch(`/api/fragments?league=${encodeURIComponent(league)}`).then(r => r.json())
  ])
  state.eco = eco
